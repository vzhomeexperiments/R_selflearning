...
create_labelled_data <- function(x, n = 15){
  ...
  for (i in 1:length(dat11)) {
    ...
    if(!exists("dfr12")){
      dfr12 <- dat11[i] %>% as.data.frame() %>% t() %>% as.tibble() %>% mutate(LABEL = ifelse(V1>V15, "BE", "BU"))} else {
        dfr12 <- dat11[i] %>% as.data.frame() %>% t() %>% as.tibble() %>% mutate(LABEL = ifelse(V1>V15, "BE", "BU")) %>% bind_rows(dfr12)
      }
